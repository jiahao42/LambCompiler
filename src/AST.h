...
inline void emit(std::string s) { std::cout << "\t" << s << std::endl; }
...
public:
    temp() { name = "t" + std::to_string(temp_index++); }
...
public:
    label() { name = "L" + std::to_string(label_index++); }
    std::string to_string() { return name; }
};
...
public:
    ...
    virtual value *code_gen() { return new value(std::to_string(val)); }
};
...
public:
    ...
    virtual value *code_gen() { return new value(name); }
};
...
public:
    binary_expr_node(std::string _op, expr_node *_lhs, expr_node *_rhs)
    ...
    virtual value *code_gen() {
        ...
        case '/': {
            temp tmp;
            emit(tmp.to_string() + " = " + L->get_val() + " " +
                 std::string(1, tmp_op) + " " + R->get_val());
...
public:
    ...
    if_stmt_node(expr_node *_cond, std::vector<expr_node *> _if_stmts,
                 std::vector<expr_node *> _else_stmts)
...
public:
    for_loop_node(expr_node *_init, expr_node *_compare, expr_node *_update,
                  std::vector<expr_node *> _body)
        : init(_init), compare(_compare), update(_update), body(_body) {}
...
public:
    while_expr_node(expr_node *_cond, std::vector<expr_node *> _while_stmts)
...
public:
    call_expr_node(const std::string &_callee, std::vector<expr_node *> &_args)
        : callee(_callee), args(_args) {}
    virtual value *code_gen() { return nullptr; }
};
...
public:
    prototype_node(const std::string &_name,
...
public:
    function_node(prototype_node *_proto, expr_node *_body)
