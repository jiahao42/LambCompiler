...
private:
    ...
    line cur_line_info;   /* Information of current line, including line number
                             and line address, See symbol_table.h */
...
public:
    ...
    void parse_identifier(
        size_t &); /* Parse identifier, start with letter or underscore */
    void parse_char(size_t &); /* Parse character, start with ', also end with
                                  ', be careful with escape ' */
    void
    parse_string(size_t &); /* Parse string, start with ", also end with ", be
                               careful with escape " */
    void parse_single_line_comment(
        size_t &); /*  Parse single line commeng, start with // */
    void parse_multi_line_comment(
        size_t &); /* Parse multi-line comment, start with *\/, also end with
                      *\/ */
};

void init_symbol_table(const char *filedir,
                       const char *filename); /* Initialize the symbol table */
